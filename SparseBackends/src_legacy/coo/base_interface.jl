...
function Base.similar(A::RestrictedCOO{T,N}, ::Type{S}, dims::NTuple{N,Int}) where {T,N,S}
  reps, rep_of = _build_rep_map(dims, A.diag_pairs)
  nps = _n_prefix_states(dims, reps)
  return RestrictedCOO{S,N}(dims, A.diag_pairs, reps, rep_of, zeros(Int, nps+1),
                           Tuple{Int,Int}[], S[])
end

Base.copy(A::RestrictedCOO{T,N}) where {T,N} =
  RestrictedCOO{T,N}(A.dims, copy(A.diag_pairs), copy(A.reps), copy(A.rep_of),
