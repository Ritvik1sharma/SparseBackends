...
# @inline function _decode_plin_to_prefix!(prefix::Vector{Int}, plin::Int, dims_prefix::Vector{Int})
#   # column-major decode, same as your _rep_decode! but for full prefix
#   x = plin - 1
#   @inbounds for a in 1:length(dims_prefix)
#     d = dims_prefix[a]
#     prefix[a] = (x % d) + 1
#     x ÷= d
#   end
...
# @inline function _plin_from_prefix(prefix::Vector{Int}, dims_prefix::Vector{Int})
#   lin = 1
#   stride = 1
#   @inbounds for a in 1:length(dims_prefix)
#     lin += (prefix[a] - 1) * stride
#     stride *= dims_prefix[a]
#   end
#   return lin
# end

# @inline function _check_diag_pairs(prefix::Vector{Int}, diag_pairs::Vector{Tuple{Int,Int}})
#   @inbounds for (a,b) in diag_pairs
