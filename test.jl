...
let
    ...
    N = 1 # 2 plaquettes = 4 sites, so 8 total spins
    ...
    # n1, n2 = norm(psi1), norm(psi2)
    # ovlp = inner(psi1, psi2)
    # norm_error = sqrt(abs(n1^2 - 2*real(ovlp) + n2^2))
    # fidelity = abs(ovlp) / (n1 * n2)
    # println(" START ‖psi - psi_old‖ = $norm_error  |⟨psi_old|psi⟩|/(‖psi‖‖psi_old‖) = $fidelity")
